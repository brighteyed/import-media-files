...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create metadata files describing albums')
    parser.add_argument('--takeout-dir', type=str,
                        help='Google Photo takeout directory')
                ...
                if media_file.lower().endswith('.mp4') or media_file.lower().endswith('.mov'):
                    if zipfile.is_zipfile(media_file):
                        try:
                            ...
                            albumdata['files'].append(process_video(tmp_file))

                        except zipfile.BadZipFile:
                            print("[ERROR] BadZipFile {0}".format(media_file))
